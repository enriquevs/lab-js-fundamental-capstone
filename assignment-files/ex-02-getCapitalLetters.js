...
function getCapitalLetters(string) {
  ...
  var array = string.split(""); // se tiliza .split para convertir el string en lista
  var newArray = []; //se crea un nuevo array

  for(var i = 0; i < array.length; i++) { //iteracion de los elementos

    if (array[i] === array[i].toUpperCase() && array[i] !== " "){ //se hace la comparacioon para sabr si son letras mayusculas
      var capitalLetter = array[i];
      newArray.push(capitalLetter); // se sube al nuevo array con .push
    }
  }
  return (newArray);
}
